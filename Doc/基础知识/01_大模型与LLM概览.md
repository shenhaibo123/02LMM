...
本知识体系聚焦于从零开始构建一个（多模态）大模型。我们以实践为导向，尽量精简数学理论，将重点放在核心概念、工程实践与关键技术上。

- **1）总览与核心概念（本篇）**
  - LLM 定义与范式演进
  - 生命周期：数据 → 预训练 → 对齐/微调 → 部署/监控
  - 核心术语：Token, 上下文, PPL, Loss

- **2）基石：分词与数据工程**
  - **Tokenizer**：文本的数字表示
    - 主流算法：BPE, WordPiece, Unigram
    - 特殊符号与词表扩展
  - **数据准备与处理**
    - 预训练语料的收集、清洗与去重
    - **序列打包（Packing）**：大幅提升训练效率的关键技巧

- **3）模型核心：Attention 与 Transformer**

  Transformer 是现代 LLM 的绝对基石。理解其核心组件，特别是 Attention 机制的演进，对于理解不同模型的性能和资源消耗至关重要。
...
- **8）实践工具链（与本仓库对应）**
  - **核心库**：Hugging Face Transformers, TRL, Accelerate
  - **训练框架**：LLaMA-Factory, DeepSpeed
  - **推理服务**：vLLM, TGI
  - **本仓库目录映射**：`dataset/`, `model/`, `trainer/`, `scripts/`

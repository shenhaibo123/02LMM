...
3. 训练完成后，确认：
   - `out/` 下是否生成了权重文件；
   - `out/logs/` 下是否有对应任务的 JSONL 或曲线数据（若脚本已集成）。
...
2. 向用户简要总结：
   ...
   - 后续推荐操作（例如继续调参、多卡训练，或参考 [Omni 仓库](https://github.com/shenhaibo123/Omni) 做全模态实战等）。

> 本 skill 不负责将数据集或模型权重推送到 GitHub；训练产生的 `out/`（含 `out/logs/`）已在 `.gitignore` 中忽略，保持仓库干净。

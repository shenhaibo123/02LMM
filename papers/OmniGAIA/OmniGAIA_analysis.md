...
- **类型**：分布图。
- **内容**：展示不同模型的工具调用次数分布。横轴为每次任务的工具调用次数（0–15+），纵轴为该调用次数对应的任务数量/比例。
- **关键发现**：Qwen3-Omni 呈现极端的"不调用"模式（大量任务 0 次调用，说明模型不会主动使用工具）；Gemini-3-Pro 呈现广泛的调用分布（1–10 次较均匀）；OmniAtlas 经过 SFT+OmniDPO 后从"不调用"转向更活跃的工具使用，但调用次数与成功率非线性——超过 10 次的调用往往伴随"乱调"行为。
- **与正文对应**：Section 5.3 工具使用行为分析。
- **解读**：该图揭示了当前全模态模型的关键瓶颈——大多数开源模型不会主动使用工具，而即使调用工具，质量也参差不齐。有效的工具使用策略（而非仅增加调用次数）是提升任务通过率的关键。

---

### Table 1：OmniGAIA 主结果——各模型 Pass@1 对比

- **列名**：Model | Modality (Video+Audio / Image+Audio) | Difficulty (Easy/Medium/Hard) | Overall。
- **行名**：闭源模型（Gemini-3-Pro 62.5、Gemini-3-Flash 38.3、GPT-4.1 23.1、Claude-4-Sonnet 18.3）和开源模型（Qwen3-Omni-30B 13.3/16.4/7.2/4.2、Qwen3-Omni-7B 11.7、Qwen2.5-Omni-7B 9.7）以及 OmniAtlas 变体。
- **关键数据**：最强模型 Gemini-3-Pro 仅 62.5%，说明基准极有挑战性。开源最佳 Qwen3-Omni-30B 仅 13.3%，与闭源差距约 5 倍。OmniAtlas 训练后提升至 20.8%（+56%），但仍远低于 Gemini-3-Pro。
- **论证作用**：证明 OmniGAIA 的高难度以及现有开源模型在全模态工具使用+推理上的严重不足。

---

### Table 2：OmniAtlas 消融实验

- **内容**：逐步对比 Base（Qwen3-Omni 13.3）→ +SFT（18.9）→ +OmniDPO（20.8）的 Pass@1 变化，以及感知/推理/工具使用/生成四类错误率的变化。
- **关键数据**：SFT 贡献最大（+5.6 pp），OmniDPO 进一步提升（+1.9 pp）并全面降低错误率（无效工具使用率从 81.1%→59.4%）。
- **论证作用**：证明树搜索轨迹合成+SFT 和细粒度纠错+DPO 的有效性。

---

### Table 3：原生感知 vs 工具感知对比

- **内容**：比较直接输入完整媒体（原生感知）和通过工具请求片段（工具感知）两种模式的性能。
- **关键数据**：对强模型（Gemini-3-Pro）原生感知更优；对弱模型（开源），工具感知可作为补充手段减少信息稀释。
- **论证作用**：验证了主动全模态感知的设计动机——对长媒体按需请求片段优于整体下采样。
